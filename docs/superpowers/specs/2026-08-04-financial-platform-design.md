...
- **`ignore_rule`** — descrições normalizadas que devem entrar ignoradas (ex.: PIX entre contas próprias, identificado pelo nome do titular). Criada/removida pelo toggle 🚫 em Transações, com aplicação retroativa a todas as transações de mesma descrição; consultada na importação. Gerenciável em Configurações.
